...
import static com.mohism.pudding.system.manager.util.ResponseUtil.resultMap;
...
public class AuthenticationFailHandler extends SimpleUrlAuthenticationFailureHandler {
    ...
    @Value("${pudding.loginTimeLimit}")
    private Integer loginTimeLimit;

    @Value("${pudding.loginAfterTime}")
    ...
    public void onAuthenticationFailure(HttpServletRequest request, HttpServletResponse response, AuthenticationException e) throws IOException, ServletException {
        ...
        if (e instanceof UsernameNotFoundException || e instanceof BadCredentialsException) {
            ...
            if(restLoginTime<=3&&restLoginTime>0){
                ResponseUtil.out(response, resultMap(false,500,"用户名或密码错误，还有"+restLoginTime+"次尝试机会"));
            } else if(restLoginTime<=0) {
                ResponseUtil.out(response, resultMap(false,500,"登录错误次数超过限制，请"+loginAfterTime+"分钟后再试"));
            } else {
            ...
            ResponseUtil.out(response, resultMap(false,500,((LoginFailLimitException) e).getMsg()));
        } else {
